---
title: "Dropped out or facility terminated from residential opioid treatment"
author: "Abby Stevens"
date: "2020-11-25"
...
**Response**: Dropped out of or facility terminated short or long-term opioid treatment.
...
mydata$dropout = ifelse(mydata$REASON %in% c(2,3), 1, 0) #create response variable
...
Not perfect, but acceptable. Next, we can vizualize some (or all) of the relationships between the features and the response. Here we just look at 3 of the variables so it's less chaotic.
...
test_roc = roc(teds_test[,response] ~ test_prob, plot = TRUE, print.auc = TRUE)
```
...
In this case, we see that the AUC=0.72 on the test set, an improvement over the logistic regression. We can use the test predictions to generate the ROC curve:
